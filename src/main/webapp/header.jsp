...
<script type="text/javascript">
	$(document).ready(function(){
		$.ajax({
			url:"${pageContext.request.contextPath}/goods/getgoodstypelist",
			type:"GET",
			success:function(data){
				for(var i in data){//i是索引
					var a = $("<a href='${pageContext.request.contextPath}/goods/getgoodsbytypeid?typeId="+data[i].id+"'>"+data[i].name+"</a>");
					$("#goodsType").append(a);
    	...
    	<div id="topdiv">
            ...
            <span style="float:right">
       			...
       			<c:if test="${not empty user}">
       				...
       				<li>|</li>
       				<a href="${pageContext.request.contextPath}/user/logout" onclick="return confirm('确定要注销吗?')" id="a_top">注销</a>
       				<li>|</li>
       				<a href="http://localhost:8080/order/getorderlist" id="a_top">我的订单</a>
       				<li>|</li>
       				<a href="${pageContext.request.contextPath}/address/getaddress" id="a_top">地址管理</a>
       			</c:if>
                <li>|</li>
                ...
                <a href="${pageContext.request.contextPath}/cart/getcart" id="shorpcar">购物车</a>
            </span>
        </div>
 </div>
<div id="second">
       ...
       <form class="form-inline pull-right" style="margin-top: 40px;margin-right: 10px;" action="${pageContext.request.contextPath }/goods/searchgoods" method="get">
		
		  <div class="form-group">
		    <%--<input type="hidden" name="method" value="searchgoods	">--%>
